...
def draw_battle(bg, fnt, obj, player): # 戦闘画面の描画 obj:戦闘で行動中のオブジェクト
    ...
    if idx == 11 or idx == 19: # コマンド選択 or モンスター選択（攻撃）
        ...
        if idx == 11:
            if tmr%5 != 0:
                draw_text(bg, "・", x_i+btl_cmd_x*x_w/2, y_i+70+btl_cmd_y*60, fnt, col)
        ...
        if idx == 19:
            if tmr%5 != 0:
                draw_text(bg, "・", x_i_t-50, y_i_t+y_h*btl_enemy, fnt, col) # btl_nenmy(モンスターの名前の位置)
        ...
        if tmr%5 != 0:
            draw_text(bg, "・", x_i_t-50+int(x_w/2)*sel_spell_x, y_i_t+y_h*sel_spell_y, fnt, col)
...
def battle_command(bg, key): # コマンドの入力と表示
    ...
    if key[K_UP] and btl_cmd_y > 0: # ↑キー
        ...
        btl_cmd_x += 1
    elif key[K_SPACE] or key[K_RETURN]: # 決定space
        ent = True
    return ent
